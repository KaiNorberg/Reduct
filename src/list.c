...
{
    if (index >= tailOffset || list->root == NULL)
    {
        return &list->tail;
    }

    reduct_list_node_t* node = list->root;
    for (uint32_t level = list->shift; level > 0; level -= REDUCT_LIST_BITS)
    {
...
static reduct_list_node_t* reduct_push_tail(reduct_t* reduct, uint32_t shift, size_t index,
    reduct_list_node_t* parent, reduct_list_node_t* tailNode)
{
    ...
    reduct_list_node_t* newNode =
        parent != NULL ? reduct_list_node_copy(reduct, parent) : reduct_list_node_new(reduct);
    if (parent == NULL)
    {
...
REDUCT_API reduct_list_t* reduct_list_new_from_handles(reduct_t* reduct, size_t count, reduct_handle_t* handles)
{
    assert(reduct != NULL);
    assert(handles != NULL || count == 0);
